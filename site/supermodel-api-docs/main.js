        ...
        if (items.length > 1) {
          ...
          for (var i = 0; i < items.length; i++) {
            ...
            if (items[i].slug && !isLast) {
              svg += '<a href="/supermodel-api-docs/' + items[i].slug + '.html">';
            }
    ...
    try {
      ...
      if (graphData && graphData.nodes && graphData.nodes.length > 1) {
        ...
        node.on("click", function(event, d) {
          if (d.slug) window.location.href = "/supermodel-api-docs/" + d.slug + ".html";
        });
    ...
    try {
      ...
      if (dist.length > 1) {
        ...
        var arcs = g.selectAll("path").data(pie(dist)).enter().append("path").attr("d", arc).attr("fill", function(d, i) { return hubColors[i % hubColors.length]; }).attr("stroke", "#000000").attr("stroke-width", 2).style("cursor", "pointer")
          .on("click", function(event, d) { window.location.href = "/supermodel-api-docs/" + bestKey + "/" + toSlug(d.data.name) + ".html"; });
        ...
        dist.forEach(function(d, i) {
          if (i >= 8) return;
          var ly = 16 + i * 22;
          var lg = svg.append("g").style("cursor", "pointer").on("click", function() { window.location.href = "/supermodel-api-docs/" + bestKey + "/" + toSlug(d.name) + ".html"; });
      ...
      if (hubSecEl && topEnts.length > 0) {
        ...
        topEnts.forEach(function(d, i) {
          ...
          var g = teSvg.append("g").style("cursor", "pointer")
            .on("click", function() { window.location.href = "/supermodel-api-docs/" + d.slug + ".html"; });
    ...
    try {
      ...
      if (entries.length > 0) {
        ...
        entries.forEach(function(d, i) {
          ...
          var g = svg.append("g").style("cursor", "pointer").on("click", function() { if (taxKey) window.location.href = "/supermodel-api-docs/" + taxKey + "/" + toSlug(d.name) + ".html"; });
    ...
    try {
      ...
      if (types.length > 0) {
        ...
        var node = svg.selectAll("g").data(root.leaves()).enter().append("g").attr("transform", function(d) { return "translate(" + d.x + "," + d.y + ")"; })
          .style("cursor", "pointer").on("click", function(event, d) { window.location.href = "/supermodel-api-docs/" + "node_type/" + toSlug(d.data.name) + ".html"; });
    ...
    try {
      ...
      if (ltEntries.length > 0) {
        ...
        ltEntries.forEach(function(d, i) {
          ...
          var label = d.name.length > 22 ? d.name.substring(0, 20) + ".." : d.name;
          var g = svg.append("g").style("cursor", "pointer").on("click", function() { if (ltKey) window.location.href = "/supermodel-api-docs/" + ltKey + "/" + toSlug(d.name) + ".html"; });
...
(function() {
  ...
  function loadIndex() {
    fetch("/supermodel-api-docs/search-index.json")
  ...
  function renderResults() {
    ...
    for (var i = 0; i < results.length; i++) {
      ...
      html += '<a href="/supermodel-api-docs/' + e.s + '.html" class="' + cls + '">';
  ...
  input.addEventListener("keydown", function(e) {
    ...
    else if (e.key === "Enter" && activeIdx >= 0 && results[activeIdx]) { e.preventDefault(); window.location.href = "/supermodel-api-docs/" + results[activeIdx].entry.s + ".html"; }
  });
